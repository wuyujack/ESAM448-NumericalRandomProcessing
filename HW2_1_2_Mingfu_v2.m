...
Std_I_hat=zeros(p,1);
M_range=zeros(p,1);
abs_error_matrix=zeros(p,N_trials);
...
mytitle1 = ['log-log plot of all the statistics of Problem 1(b) when alpha =', num2str(alpha)];
title(mytitle1);

figure;
loglog(M_range,average_I_hat,'-s');
mytitle4 = ['log-log plot of average I hat when alpha =', num2str(alpha)];
title(mytitle4);
xlabel('$M$','Interpreter','latex','FontSize',13)
...
figure;
loglog(M_range,Sigma_hat_I_mean,'-s');
mytitle3 = ['log-log plot of the average of the standard error of the mean when alpha =', num2str(alpha)];
title(mytitle3);
xlabel('$M$','Interpreter','latex','FontSize',13)
...
figure;
loglog(M_range,Std_I_hat,'-s');
mytitle2 = ['log-log plot of standard deviation of I hat when alpha =', num2str(alpha)];
title(mytitle2);
xlabel('$M$','Interpreter','latex','FontSize',13)
